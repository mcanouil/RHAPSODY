...
params:
  ...
  warning: FALSE
  message: FALSE
title: 'RHAPSODY WP3 Pre-Diabetes Analysis Plan (Version 0.8.4)'
...
format_lb <- function(data) {
  data <- data %>%
    mutate(
      LBFAST = ifelse(grepl("LBFAST", colnames(.)), LBFAST, NA),
      LBTPTNUM = ifelse(grepl("LBTPTNUM", colnames(.)), LBTPTNUM, NA),
...
get_residuals_outliers <- function(x) {
  x@frame %>% 
    ...
    filter(between(residuals, r_median-r_iqr, r_median+r_iqr)) %>% 
    .[["SUBJID"]]
    return()
}
...
for (ivariable in variables_names) {
  ...
  if (!all(is.na(fit_data[, ivariable]))) {
    ...
    if (any(grepl("PVALUE", colnames(fit_fixed)))) {  
      pretty_kable(fit_fixed, pval_cols = grep("PVALUE", colnames(fit_fixed))) %>%
        print()
    } else {
      cat("Warning: p-values were not computed by lmerTest, probably due to computational error.")
      cat("\n")
...
pretty_kable(descriptive_table)
```
...
for (ichrdirectory in list_chrom_directories) {
  ...
  for (idosagefile in list_dosage_file) {
    ...
    results_lmm_chunk <- merge(
      x = annotation_chunk,
      y = do.call("rbind", results_lmm_snp),
      by = "SNP"
    )
    ...
    for (ivariable in variables_names) {
      if (!grepl("_NA", ivariable)) {
        ...
        write_csv(
          x = filter(results_lmm_chunk, TRAIT == ivariable),

...
params:
  ...
  warning: FALSE
  message: FALSE
title: 'RHAPSODY WP3 Pre-Diabetes Analysis Plan (Version 1.0.3)'
...
# Run the analysis
dir.create(path = working_directory, recursive = TRUE, showWarnings = FALSE, mode = '0777')
rmarkdown::render(
...
list.files(
  ...
  pattern = ".*.vcf.gz$", # '.*[2][12]+' part to be removed later
  full.names = TRUE
) %>% 
...
do
  ...
  ## unzip vcf and keep all but header. Split the files into lines of 10000 and call them pre*
  ...
  filter(
    SUBJID%in%(
      grep(
        ...
        .[1] %>% 
        list.files(pattern = ".DS.gz$", full.names = TRUE, nrows = 100) %>% 
        .[1] %>% 
...
for (ichrdirectory in list_chrom_directories) {
  ...
  for (idosagefile in list_dosage_file) {
    ...
    results_lmm_snp <- mclapply(
      X = head(colnames(dosage_file_tidy), 80),
      mc.cores = params$n_cpu,
      mc.allow.recursive = FALSE,
      mc.preschedule = FALSE,
      mc_variables_names = variables_names,
      mc_pheno_geno_data = pheno_geno_data,
      FUN = function(isnp, mc_variables_names, mc_pheno_geno_data) {
        results_lmm <- mc_variables_names %>% 
          length() %>% 
          rep(NA, .) %>% 
          as.list() %>% 
          `names<-`(mc_variables_names)
        for (ivariable in mc_variables_names) {
          if (!all(is.na(mc_pheno_geno_data[, ivariable]))) {
            ...
            fit_lmm <- lmer(formula = local_formula, data = local_data)
            ...
            colnames(results_lmm[[ivariable]]) <- gsub(
              pattern = paste0("`", isnp, "`"),
              replacement = "SNP",
              x = colnames(results_lmm[[ivariable]]),
              fixed = TRUE
            )
          } else {
    ...
    results_lmm_chunk_output <- results_lmm_chunk %>% 
      rename(imputation_quality_INFO = !!params$imputation_quality_tag) %>% 
      transmute(
        TRAIT = TRAIT,
        SNPID = SNP,
...
csv_header <- paste0(tempfile(), ".csv")
write_csv(
  x = cbind.data.frame(
    ...
    "AF_coded_all", "HWE_pval", "n_total", "imputation_quality_INFO"
  ), 
  path = csv_header, 
  col_names = FALSE
)
...
concatenate_chunks <- tibble(
  ...
  unnest(chr_directories) %>% 
  mutate(
    trait_directories = map(
  ...
  unite(col = file, output_lmm, chr_directories, trait_directories, chunk_file, sep = "/") %>% 
  select(-output_directory) %>% 
  group_by(trait, chr) %>% 
  nest() %>% 
  mutate(
    concatenate = map(.x = data, .f = function(.data) {
      cmd <- paste(
        "cat",
...
concatenate_chunks %>% 
  unnest(data) %>% 
  slice(1) %>% 
  select(results_filename) %>% 
  .[[1]] %>% 
  read_csv(file = ., col_names = TRUE, n_max = 20) %>% 

...
title: 'RHAPSODY WP3 Pre-Diabetes Analysis Plan (Version 0.6.3)'
...
mykable <- function (
    ...
    if (!is.null(pval_cols)) {
        data[, pval_cols] <- base::format.pval(
            pv = data[, pval_cols], 
...
convertHbA1c <- function (x, unitFrom = "%") {
    if (unitFrom== "%") {
        HBA1Cconvert <- structure(list(
            V1 = c(10L, 12L, 14L, 16L, 18L, 20L, 22L, 24L,
            ...
            V2 = c(3.1, 3.2, 3.4, 3.6, 3.8, 4, 4.2, 4.3, 4.5, 4.7, 4.9,
        ...
        return(sapply(x, function (y) {round(sum(tidy(lm(V1~V2, data =  HBA1Cconvert))[, "estimate"] *c(1, y)))}))
    } else {
        return(x)
    }
}
...
formatTableLB <- function (data) {
    ...
    data0 <- merge(
        ...
        y = (data %>%
                 select(setdiff(colnames(.), c("LBORRES", descVariables))) %>%
                 distinct()),
        by = "KeyFactor",
        all = TRUE
    ) %>% 
...
2.  For fasting and 2-h glucose, ensure only those participants who are fasted for at least 8-h are included.
    ...
    > **Paul Franks:** We should consider if we want to include participants who have fasted for less than 8-h.  
    Having these in will probably introduce error, but is unlikley to bias the results.  
    However, if we do this, we should request analyses with and without the participants who have not fully fasted, for sensitivity analsyes.
    >
    > **Azra Kurbasic:** There is literature showing that if they fasted at least threee hours then there is no difference.  
    Analysis are pretty time consuming, hence we should aviod sensitivity analysis if possible!
...
5.  Exclusion of participants developing diabetes during the study time:
    * Individuals developing diabetes during the follow up and with no available information on medication  should be excluded.
        ...
        > **Dorte Vistisen:** I think we should at least do a sensitivity analysis including them.  
        Some of them are probably "true" fast-progressors (i.e. they progress to a diabetic value of the trait studied). 
        > 
        > **Paul Franks:** We've discussed this at length in DIRECT.  
        The problem with excluding incident diabetes, is that these are probably the people with the highest biomarker burden, so we would bias our discovery efforts by excluding them.  
        The absence of data on medication could also be a problem for people who appear to be prediabetic, if the diagnostic label is missing for some reason. 
        >
        > **Azra Kurbasic:** I've looked into big file with variable list and it is not complete as information for medication availability for CoLaus and Addition is missing.  
        We need to discuss with somebody from the clinic about medication adjustment!  
        If possible, we should adjust for medication otherwise exclude all with diagnosed diabetes!
...
> **Azra Kurbasic:** What is more reasonable, to put MAF threshold to 1% or use MAF count?
> 
> **Paul Franks:** Why not use MAC (perhaps tis is what you mean by "MAF count"?
Yes, MAF count is same as MAC.  
When we put together numbers from all the cohorts we come up to ca 20,000 individuals, a couple of thousand have only metabochip data.  
I never take decision on this matter hence the question, waht MAC do you suggest.  
In GIANT study they put limit on count being $2\times MAF\times N$ (https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4083217/).
...
for (ichrdirectory in listChrDirectories) {
    ...
    for (idosagefile in listDosageFile) {
        ...
        listerrors <- sapply(resLMM.snp, function (el) {class(el)=="try-error"})
...
# subset(resLMM.chunk, TRAIT=="GLUC") %>% 
#     head(., 20) %>% 
#     mykable()
listChrDirectories <- grep(
    pattern = "extractVCF", 

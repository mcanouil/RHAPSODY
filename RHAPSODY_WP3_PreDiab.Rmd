...
params:
  ...
  warning: FALSE
  message: FALSE
title: 'RHAPSODY WP3 Pre-Diabetes Analysis Plan (Version 1.1.0)'
...
samples_with_insulin <- filter(.data = check_medication, CMCAT%in%"INSULINS AND ANALOGUES")[["ID"]]
...
FollowUpDiabetesSamples.ToExclude <- setdiff(
  x = unique(c(follow_up_diabetes_samples_def, follow_up_diabetes_samples_diag)),
...
nTimePoints <- nrow(distinct(select(.data = DMVSLB_QC3)))
  ...
  if (!all(is.na(fit_data[, ivariable]))) {
    ...
    if (nTimePoints > 2) {
      ...
      fit_random <- as.data.frame(
        column_to_rownames(
          df = mutate(.data = fit_random, Mod = c("Mod0", "Mod1")),
          var = "Mod"
        )
      )
      print(
...
DMVSLB_QC4 <- filter(.data = DMVSLB_QC3, !SUBJID%in%unique(unlist(residuals_outliers)))
```
...
list_files <- list.files(
  ...
  pattern = '.*[2][12]+.*.vcf.gz$', # '.*.vcf.gz$', # '.*[2][12]+.*.vcf.gz$' part to be removed later
  full.names = TRUE
)
...
n_total <- unique(DMVSLB_QC5[["SUBJID"]])
...
for (ichrdirectory in list_chrom_directories) {
  ...
  for (idosagefile in list_dosage_file[c(1, 2)]) {
    message(idosagefile)
    ...
    pheno_geno_data <- full_join(
      x = DMVSLB_QC5,
      y = mutate(
        .data = rownames_to_column(df = dosage_file_tidy, var = "SUBJID"), 
    ...
    results_lmm_snp <- mclapply(
      ...
      FUN = function(isnp, mc_variables_names, mc_pheno_geno_data) {
        results_lmm <- as.list(rep(NA, length(mc_variables_names))) 
        names(results_lmm) <- mc_variables_names
        for (ivariable in mc_variables_names) {
          if (!all(is.na(mc_pheno_geno_data[, ivariable]))) {
            ...
            results_lmm[[ivariable]] <- cbind.data.frame(
              TRAIT = ivariable, 
              SNP = isnp, 
              # n_alleles = sum(distinct(local_data[, c("SUBJID", isnp)])[[isnp]]),
    ...
    annotation_chunk <- full_join(
      x = mutate(
        .data = read_tsv(gsub(".DS.gz", ".frq", idosagefile)),
        POS = as.character(POS)
      ),
      y = mutate(
        .data = read_tsv(gsub(".DS.gz", ".hwe", idosagefile)),
        POS = as.character(POS)
      ), 
    ...
    annotation_chunk <- full_join(
      x = annotation_chunk,
      y = mutate(
        .data = read_tsv(gsub(".DS.gz", ".INFO", idosagefile)),
        POS = as.character(POS)
      ), 
    ...
    results_lmm_chunk <- full_join(
      x = annotation_chunk,
      y = bind_rows(results_lmm_snp),
      by = "SNP"
    )
    
    results_lmm_chunk_output <- rename(
    ...
    results_lmm_chunk_output <- transmute(
...
count_samples <- function(data) { 
  nrow(distinct(select(.data = data, STUDYID, SUBJID)))
}

...
params:
  ...
  message: FALSE
  debug: FALSE
title: "RHAPSODY WP3 Pre-Diabetes Analysis Plan (Version 1.2.10)"
...
get_residuals_outliers <- function(x) {
  dplyr::filter( 
    ...
    tidyr::unite(col = "ID", c("STUDYID", "FAMILYID", "SUBJID"), sep = "_")
}
...
Total individuals available : `r ifelse(params$analysis_step>=2, DMVSLB_QC0 %>% dplyr::select(STUDYID, FAMILYID, SUBJID) %>% dplyr::distinct() %>% nrow() %>% format(., big.mark = ","), NA)`.
...
Total individuals available : `r ifelse(params$analysis_step>=2, DMVSLB_QC1 %>% dplyr::select(STUDYID, FAMILYID, SUBJID) %>% dplyr::distinct() %>% nrow() %>% format(., big.mark = ","), NA)`.
...
Total individuals available : `r ifelse(params$analysis_step>=2, DMVSLB_QC2 %>% dplyr::select(STUDYID, FAMILYID, SUBJID) %>% dplyr::distinct() %>% nrow() %>% format(., big.mark = ","), NA)`.
...
Total individuals available after QC: `r ifelse(params$analysis_step>=2, DMVSLB_QC3 %>% dplyr::select(STUDYID, FAMILYID, SUBJID) %>% dplyr::distinct() %>% nrow() %>% format(., big.mark = ","), NA)`
...
nTimePoints <- max(table(DMVSLB_QC3[c("FAMILYID", "SUBJID")]))

# Include random slope if more than two time points
if ("FAMILYID" %in% colnames(DMVSLB)) {
...
for (ivariable in variables_names) {
  ...
  if (!all(is.na(fit_data[[ivariable]]))) {
    ...
    pTraj <- dplyr::mutate(
      .data = fit_data,
      ID = paste(STUDYID, FAMILYID, SUBJID, sep = "_"),
      pT2D = ifelse(
...
DMVSLB_QC4 <- DMVSLB_QC3 %>% 
  dplyr::mutate(ID = paste(STUDYID, FAMILYID, SUBJID, sep = "_")) %>%
  dplyr::filter(!ID%in%unique(unlist(residuals_outliers))) %>%
  dplyr::select(-ID)
```

Total individuals available : `r ifelse(params$analysis_step>=3, DMVSLB_QC4 %>% dplyr::select(STUDYID, FAMILYID, SUBJID) %>% dplyr::distinct() %>% nrow() %>% format(., big.mark = ","), NA)`.
...
descriptive_table <- DMVSLB_QC4 %>%
  ...
  dplyr::mutate(
    NumberParticipants = format(length(unique(ID)), big.mark = ","),
...
count_samples <- function(data) { 
  nrow(dplyr::distinct(dplyr::select(.data = data, STUDYID, FAMILYID, SUBJID)))
}

...
params:
  ...
  warning: FALSE
  message: FALSE
title: 'RHAPSODY WP3 Pre-Diabetes Analysis Plan (Version 1.2.0)'
...
tidy_lmer <- function(x) {
  ...
  res <- gather(data = res, key = "Variable", value = "Value", -"Term")
  rownames(res) <- paste(res[, "Term"], res[, "Variable"], sep = "_")
  return(data.frame(t(res[, "Value", drop = FALSE]), check.names = FALSE, stringsAsFactors = FALSE))
}
...
trash <- map(.x = list_chrom_directories, .f = function(ichrdirectory) {
  ...
  trash <- map(.x = list_dosage_file, .f = function(idosagefile) {
    message(idosagefile)
    log_file <- paste0(output_lmm_directory, "/", gsub(".*/([^.]*).*", "\\1", idosagefile), ".log")
    if (file.exists(log_file)) {
      message(
        paste0('Chunk "', basename(idosagefile), '" from "', ichrfilename, '" was already analysed.', '\n'),
        paste0('Chunk "', basename(idosagefile), ' is skipped.')
      )
      return(NULL)
    }
    ...
    results_lmm_snp <- mclapply(
      ...
      FUN = function(isnp, mc_variables_names, mc_pheno_geno_data) {
        map_df(.x = mc_variables_names, .f = function(ivariable) {
    ...
    ## Write results
    trash <- map(.x = variables_names, .f = function(ivariable) {
  ...
  rm(
    list = c(
      # "ichrfilename",
      # "output_lmm_directory",
      # "list_dosage_file",
      # "idosagefile",
      "dosage_file",
      # "samplesWithPhenoGeno",
      "dosage_file_tidy",
      # "multi_allele_snps",
      "pheno_geno_data",
      "results_lmm_snp",

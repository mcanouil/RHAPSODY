...
params:
  ...
  warning: FALSE
  message: FALSE
title: 'RHAPSODY WP3 Pre-Diabetes Analysis Plan (Version 1.0.6)'
...
tidy_lmer <- function(x) {
  res <- coefficients(summary(x))
  colnames(res) <- c("ESTIMATE", "SE", "DF", "STAT", "PVALUE")
  rownames(res) <- gsub(".*Intercept.*", "Intercept", rownames(res))
  return(
    as.data.frame(
      t(
        column_to_rownames(
          df = unite(
            data = gather(
              data = rownames_to_column(df = as.data.frame(res), var = "Term"),
              key = "Variable", value = "Value", -"Term"
            ), 
            col = Term, Term, Variable, sep = "_"
          ),
          var = "Term"
        )
      )

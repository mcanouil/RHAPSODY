...
params:
  ...
  warning: FALSE
  message: FALSE
title: 'RHAPSODY WP3 Pre-Diabetes Analysis Plan (Version 1.2.2)'
...
pretty_kable <- function(
  ...
  if (knitr:::is_latex_output()) {
    ...
    kableExtra::kable(x = data, format.args = format.args, col.names = col.names, ...) %>%
      kableExtra::kable_styling(
...
convert_hba1c <- function(x, unitFrom = "%") {
  if (unitFrom == "%") {
    ...
    hba1c_model <- stats::coef(stats::lm(V1~V2, data = hba1c_unit_table))
...
get_residuals_outliers <- function(x) {
  dplyr::filter( 
    .data = dplyr::mutate(
      ...
      r_iqr = 3 * stats::IQR(residuals),
      r_median = stats::median(residuals),
      left = r_median-r_iqr,
...
compute_mean_sd <- function(x, digits = 2) {
  paste0(
    ...
    " (",
    round(stats::sd(x, na.rm = TRUE), digits = digits),
    ")"
  )
}
```
...
one_time_point_samples <- DMVSLB_QC0 %>%
  ...
  dplyr::filter_at(
    .vars = dplyr::vars(!!!variables_names_available),
    .vars_predicate = dplyr::any_vars(. <= 1)
  ) %>%
  dplyr::select(STUDYID, SUBJID) %>%
  dplyr::mutate(ID = paste(STUDYID, SUBJID, sep = "_")) %>%
  dplyr::ungroup() %>%
...
samples_with_insulin <- dplyr::filter(.data = check_medication, CMCAT%in%"INSULINS AND ANALOGUES")[["ID"]]
...
for (ivariable in variables_names) {
  ...
  fit_data <- dplyr::select(
    .data = DMVSLB_QC3, 
    dplyr::matches(ivariable), AGE0, SEX0, BMI0, VISIT_YEARS, SUBJID, STUDYID
  )
  ...
  if (!all(is.na(fit_data[, ivariable]))) {
    ...
    cat("\n##### Trajectories\n\n")
    pTraj <- dplyr::mutate(.data = fit_data,
    ...
    p0 <- cowplot::plot_grid(
      ggplot2::ggplot(
        data = data.frame(x = stats::fitted(fit), y = stats::residuals(fit)), 
        mapping = ggplot2::aes(x = x, y = y)
      ) +
        ggplot2::geom_point(shape = 21, colour = viridis::viridis_pal()(1)) +
        ggplot2::labs(x = "Fitted", y = "Residuals"),
      ggplot2::ggplot(
        data = data.frame(y = stats::residuals(fit), x = "x"), 
        mapping = ggplot2::aes(x = x, y = y)
      ) +
        ...
        ggplot2::geom_hline(
          yintercept = stats::median(stats::residuals(fit)) + 3 * stats::IQR(stats::residuals(fit)),
          linetype = 2,
    ...
    if (nTimePoints > 2) {
      ...
      fit_random <- stats::anova(
        stats::update(fit, formula = .~. - (1 + VISIT_YEARS | SUBJID) + (1 | SUBJID)),
        fit, refit = FALSE
      ) 
      fit_random <- as.data.frame(
        tibble::column_to_rownames(
          df = dplyr::mutate(.data = fit_random, Mod = c("Mod0", "Mod1")),
          var = "Mod"
        )
      )
      print(
...
DMVSLB_QC4 <- dplyr::filter(.data = DMVSLB_QC3, !SUBJID%in%unique(unlist(residuals_outliers)))
```
  ...
  if (!all(is.na(fit_data[[ivariable]]))) {
    ...
    pTraj <- dplyr::mutate(
      ...
      ggplot2::ggplot(
        ...
        ggplot2::geom_line(na.rm = TRUE) +
        ggplot2::guides(colour = "none") +
        lmerTest::lmer(discrete = TRUE, begin = 0, end = 4 / 5) +
        ggplot2::facet_grid(.~pT2D)
    print(pTraj)
    cat("\n")
...
DMVSLB_QC5 %>%
  dplyr::select(!!unname(variables_names), AGE0, SEX0, BMI0, VISIT_YEARS, SUBJID, STUDYID) %>% 
  tidyr::gather(key = "Trait", value = "Measure", -c(AGE0, SEX0, BMI0, VISIT_YEARS, SUBJID, STUDYID)) %>% 
  dplyr::mutate(
    ID = paste(STUDYID, SUBJID, sep = "_"),
    pT2D = ifelse(
      ID%in%diabetic_samples_with_medication_info,
...
trash <- purrr::map(.x = list_chrom_directories, .f = function(ichrdirectory) {
  ...
  trash <- purrr::map(.x = list_dosage_file, .f = function(idosagefile) {
    message(idosagefile)
    ...
    pheno_geno_data <- dplyr::full_join(
      x = DMVSLB_QC5,
      y = dplyr::mutate(
        .data = tibble::rownames_to_column(df = dosage_file_tidy, var = "SUBJID"), 
    ...
    results_lmm_snp <- parallel::mclapply(
    ...
    ## Write results
    trash <- purrr::map(.x = variables_names, .f = function(ivariable) {
...
concatenate_chunks <- dplyr::tibble(
  ...
  tidyr::unite(col = file, output_lmm, chr_directories, trait_directories, chunk_file, sep = "/") %>% 

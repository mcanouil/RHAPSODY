...
params:
  ...
  warning: FALSE
  message: FALSE
title: 'RHAPSODY WP3 Pre-Diabetes Analysis Plan (Version 0.8.0)'
...
params <- list(
  cohort_name = 'CohortName',
  author_name = 'Firstname LASTNAME',
  opal_credentials = './Scripts/BuildScripts/opal_DESIR.txt',
  vcf_input_directory = './vcfs',
  imputation_quality_tag = 'INFO',
  vcftools_binary_path = '/disks/RAW/Softwares/vcftools/vcftools_latest/bin',
  output_directory = './',
  analysis_step = 2,
  format_vcfs = TRUE,
  variants_analysis = TRUE,
...
OneTimePointSamples <- DMVSLB.QC0 %>%
  group_by(STUDYID, SUBJID) %>%
  summarise_at(
    .vars = vars(variables_names),
    .funs = (function(.x) {sum(!is.na(.x))})
  ) %>%
  filter_at(
    .vars = vars(variables_names[-grep("_NA", variables_names)]),
